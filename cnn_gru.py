...
class resnet_gru(torch.nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(gru_in_dim, gru_out_dim, batch_first=True)
        self.fc = nn.Linear(gru_out_dim, 7)
        self.fc2 = nn.Linear(2048, 7)
        init.xavier_normal(self.gru.all_weights[0][0])
        init.xavier_normal(self.gru.all_weights[0][1])

    def forward(self, x):
        x = self.share.forward(x)
        x = x.view(-1, 2048)
        z = self.fc2(x)
        x = x.view(-1, sequence_length, gru_in_dim)
        self.gru.flatten_parameters()
        y, _ = self.gru(x)
        y = y.contiguous().view(-1, gru_out_dim)
        y = self.fc(y)
        return z, y
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    model_name = "cnn_gru_epoch_" + str(epochs) + "_length_" + str(
    ...
    record_name = "cnn_gru_epoch_" + str(epochs) + "_length_" + str(
        sequence_length) + "_opt_" + str(optimizer_choice) + "_batch_" + str(train_batch_size) + "_train1_" + str(
