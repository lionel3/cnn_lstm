...
parser = argparse.ArgumentParser(description='dense_lstm training')
...
class dense_lstm(torch.nn.Module):
    def __init__(self):
    ...
    def forward(self, x):
        x = self.relu(self.features.forward(x))
        x = self.avgpool(x)
        x = x.view(-1, 1024)
        z = self.fc2(x)
        x = x.view(-1, sequence_length, 1024)
